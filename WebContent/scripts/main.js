(function() { //immediately executed
  ...
  function init() {
    ...
    document.querySelector('#login-link').addEventListener('click', onSessionInvalid);
   ...
   * Session
   */
  function validateSession() { //before login
    ...
    // make AJAX call
    ajax('GET', url, req,   //login-doGet: validate session
      // session is still valid
      function(res) {
        var result = JSON.parse(res);

        if (result.status === 'OK') { //backend: check session
  ...
  //by default
  function onSessionInvalid() {   //session无效，show login界面，有效show主页面
    var loginForm = document.querySelector('#login-form');
    var signupForm = document.querySelector('#signup-form');
    var itemNav = document.querySelector('#item-nav');
    var itemList = document.querySelector('#item-list');
  ...
  function initGeoLocation() {   //get your own Geo location 浏览器window下
    if (navigator.geolocation) { //navigator is from window
  ...
  function onPositionUpdated(position) {  //更新lat，lon，position定义geolocation api
...
//-----------------------------------
  ...
  function ajax(method, url, data, successCallback, errorCallback) {
    ...
    xhr.onload = function() { //onload相当于ready status（1 2 3 4）为4，request回来，不一定成功还是失败
      if (xhr.status === 200) { // status of response 
    ...
    xhr.onerror = function() { //onerror request发送时有问题
    ...
    if (data === null) { // data = {}
      ...
      xhr.setRequestHeader("Content-Type",
        "application/json;charset=utf-8"); //配置json格式
      xhr.send(data);
    }
  }
...
//    itemListEl.style.marginRight = "12%";
  ...
  function addItem(itemList, item) {
    ...
    favLink.appendChild($create('i', {
      id: 'fav-icon-' + item_id,
      className: li.dataset.favorite === "true" ? 'fa fa-heart' : 'fa fa-heart-o'
    }));
